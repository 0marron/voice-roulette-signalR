...
$(".callButton").css({ "background-color": "#ccddc7" });
...
$(document).ready(function () {
    ...
    $(document).on('click', '.callButton', function () {
        ...
        if (IsCalling) {
            ...
            IsCalling = false;
            $(".callButton").css({ "background-color": "#ccddc7" });
            $(".callButton").html("<span>Начать поиск</span>");
            ...
            if (!IsCalling) {
             ...
             $(".callButton").css({ "background-color": "yellow" });
            $(".callButton").html("<span>Идет поиск...</span>");

                IsCalling = true;
                wsconn.invoke("SetSex", { "MySex": GlobalMySex, "YourSex": GlobalYourSex });


                wsconn.invoke('callUser', { "MySex": GlobalMySex, "YourSex": GlobalYourSex });
    ...
    $('.hangup').click(function () {
        ...
        var audio = new Audio("/endcall.wav");
        audio.play();
...
wsconn.on('callAccepted', (acceptingUser) => {
    ...
    $(".callButton").html("<span>Закончить звонок</span>");
    $(".callButton").css({ "background-color": "red" });
    var audio = new Audio("/begincall.wav");
    audio.play();
...
const generateRandomUsername = () => {
    ...
    alertify.success('Выберете пол и нажмите поиск ' );
    setUsername(username);
};
